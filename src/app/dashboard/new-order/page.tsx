...
const PlatformIcon = ({ name, selected }: { name: string; selected?: boolean }) => {
  const icons: Record<string, React.ReactNode> = {
    ...
    Twitter: (
      <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill={selected ? "#ffffff" : "#0f172a"}>
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
    Threads: (
      <svg className="w-4 h-4 shrink-0" viewBox="0 0 512 512" fill={selected ? "#ffffff" : "#0f172a"}>
    ...
    Discord: (
      <svg className="w-4 h-4 shrink-0" viewBox="0 0 512 388.049" fill="none">
  ...
  return (
    <div ref={containerRef} className="relative w-full" style={{ zIndex: open ? 50 : "auto" }}>
      ...
      <button
        ...
        className={`w-full flex items-center justify-between gap-2.5 rounded-2xl border px-4 h-12 text-sm transition-all cursor-pointer ${
          disabled
            ? "opacity-50 cursor-not-allowed bg-slate-100 border-slate-200 text-slate-400"
            : open
            ? "border-primary bg-white text-slate-900 shadow-[0_0_0_4px_rgba(59,130,246,0.12)]"
            : "border-slate-200 bg-slate-50 text-slate-900 hover:border-slate-300 hover:bg-white shadow-2xs"
        }`}
      >
        <span className={`truncate text-left flex-1 font-medium ${!selected ? "text-slate-400" : "text-slate-900 font-semibold"}`}>
      ...
      {open && !disabled && (
        <div
          className="absolute left-0 right-0 top-[calc(100%+8px)] bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150"
          style={{ zIndex: 9999 }}
        >
          {/* Search box */}
          <div className="p-2.5 border-b border-slate-100 bg-slate-50/80">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-white border border-slate-200 rounded-xl pl-10 pr-3.5 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 transition-all font-medium"
              />
            </div>
          </div>
          ...
          <div className="overflow-y-auto max-h-80 divide-y divide-slate-100">
            {filtered.length === 0 ? (
              <p className="text-slate-400 text-sm text-center py-8 font-medium">No results found</p>
            ) : (
              filtered.map((opt) => (
                <button
                  key={opt.value}
                  ...
                  className={`w-full text-left px-4 py-3 hover:bg-slate-50 transition-colors cursor-pointer flex items-center justify-between gap-3 ${
                    opt.value === value ? "bg-primary/5 text-primary" : "text-slate-800"
                  }`}
                >
                  <div className="min-w-0 flex-1">
                    <span className={`block text-sm leading-snug truncate ${
                      opt.value === value ? "font-bold text-primary" : "font-semibold text-slate-900"
                    }`}>
                      {opt.label}
                    </span>
                    {opt.sublabel && (
                      <span className="block text-xs text-slate-500 mt-0.5 font-medium">{opt.sublabel}</span>
                    )}
                  </div>
                  {opt.value === value && (
...
export default function NewOrderPage() {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      ...
      if (result.data) {
        ...
        if (!data.can_fulfill) {
          setProviderCheckError(
            data.message || "Apologies, an update is in effect. Please refresh the page or choose another service."
          );
        }
      }
    }, 500);
  ...
  const handleOrderSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidUrl(orderLink)) {
      setOrderError("Please enter a valid URL (e.g. https://instagram.com/...)");
      return;
    }
    ...
    if (result.data) {
      setOrderSuccess("Order placed successfully! Redirecting to orders review...");
      setOrderLink("");
      setOrderQuantity(selectedService.min_quantity.toString());
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      {/* Page Header */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">New Order</h1>
        <p className="text-slate-500 text-xs sm:text-sm mt-1 font-medium">
          Choose a platform, select your service, and launch your campaign instantly.
        </p>
      </div>
      ...
      {/* Main Order Card (100% Light Theme Only) */}
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-5 sm:p-7">
        {/* Important Guidelines Notice */}
        <div className="mb-6 p-4 sm:p-5 rounded-2xl bg-amber-50/80 border border-amber-200/90 flex items-start gap-3.5">
          <span className="w-9 h-9 rounded-xl bg-amber-100 text-amber-800 flex items-center justify-center shrink-0 mt-0.5 shadow-2xs">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </span>
          <div className="flex-1 text-xs text-amber-950">
            <h4 className="font-black text-amber-950 text-xs sm:text-sm uppercase tracking-wide mb-1.5">
              Important Order Guidelines
            </h4>
            <ul className="space-y-1 list-disc pl-4 font-semibold text-amber-900/90 leading-relaxed">
              <li>Keep your profile/target account <strong>PUBLIC</strong> until the order completes.</li>
              <li>Always double-check your link format before submitting.</li>
              <li>Please allow up to 48 hours for large follower or engagement deliveries.</li>
            </ul>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <form onSubmit={handleOrderSubmit} className="space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                {selectedCategory && servicesInCategory.length > 0 && (
                  <span className="text-[11px] font-semibold text-slate-400">
                    {servicesInCategory.length} services in category
                  </span>
                )}
              </div>
              <SearchableDropdown
            ...
            {/* Selected Service Details Plaque */}
            {selectedService && (
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 mb-2.5">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {selectedService.has_cancel && (
                      <span className="px-2.5 py-1 rounded-lg bg-amber-50 border border-amber-200 text-amber-700 text-xs font-bold">
                        ❌ Cancel Allowed
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-slate-600 font-semibold bg-white border border-slate-200 px-2.5 py-1 rounded-lg">
                    ...
                    <span>Avg Time: <strong className="text-slate-900">{selectedService.avg_completion_time || "Instant"}</strong></span>
                  </div>
                </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={selectedService ? "https://..." : "Select a service first"}
                  disabled={!selectedService}
                  className="w-full h-12 pl-11 pr-4 rounded-2xl border border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400 text-sm font-medium focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all disabled:opacity-50 disabled:bg-slate-100 disabled:cursor-not-allowed"
                />
              </div>
            </div>
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {selectedService && (
                  <span className="text-[11px] font-semibold text-slate-500">
                    Min: <strong className="text-slate-800">{selectedService.min_quantity.toLocaleString()}</strong> · Max: <strong className="text-slate-800">{selectedService.max_quantity.toLocaleString()}</strong>
                  </span>
                )}
              </div>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={selectedService ? `${selectedService.min_quantity}` : "0"}
                  disabled={!selectedService}
                  className="w-full h-12 pl-11 pr-4 rounded-2xl border border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400 text-sm font-bold focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all disabled:opacity-50 disabled:bg-slate-100 disabled:cursor-not-allowed"
                />
              </div>
            </div>
            ...
            {requiresComments && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-700">
                    Custom Comments <span className="text-primary font-black">*</span>
                  </label>
                  <span className="text-[11px] font-semibold text-slate-500">
                    {orderComments ? orderComments.split("\n").filter((l) => l.trim()).length : 0} of {orderQuantity || 0} lines
                  </span>
                </div>
                <textarea
                  value={orderComments}
                  onChange={(e) => setOrderComments(e.target.value)}
                  className="w-full min-h-30 p-4 rounded-2xl border border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-400 text-sm font-medium focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all resize-y disabled:opacity-50"
                  placeholder={"Enter each comment on a new line\nExample:\nGreat post!\nLove this content!\nAmazing work!"}
                  disabled={!selectedService}
                />
                <p className="text-slate-400 text-xs mt-1.5 font-medium">
                  Enter exactly one comment per line. The number of lines must match your quantity.
                </p>
              </div>
            )}

            {/* Price Summary & Balance Status */}
            {selectedService && (
              <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200">
                <div className="flex items-center justify-between text-xs font-semibold text-slate-500 mb-2">
                  <span>Rate per 1,000</span>
                  <span className="text-slate-800 font-bold">{formatCurrency(selectedService.user_rate)}</span>
                </div>
                <div className="flex items-center justify-between text-xs font-semibold text-slate-500 mb-2">
                  <span>Selected Quantity</span>
                  <span className="text-slate-800 font-bold">{parseInt(orderQuantity).toLocaleString() || 0}</span>
                </div>
                  ...
                  <div>
                    ...
                    {user && (
                      <div className="text-[11px] font-medium text-slate-500 mt-0.5">
                        Your Balance:{" "}
                        <strong className="text-slate-800">{formatCurrency(user.balance || 0)}</strong>
                        {parseFloat(user.balance || "0") < orderPrice && (
                          <Link href="/dashboard/wallet" className="text-rose-600 font-bold ml-1.5 hover:underline">
                            (Top up required)
                          </Link>
                        )}
                      </div>
            ...
            {/* Duplicate Order Warning Card */}
            <div className="p-4 sm:p-5 rounded-2xl bg-slate-50 border border-slate-200 flex items-start gap-3">
              <span className="w-8 h-8 rounded-xl bg-slate-200/70 text-slate-600 flex items-center justify-center shrink-0 mt-0.5">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </span>
              <div className="text-xs text-slate-600 leading-relaxed font-medium">
                <p className="font-bold text-slate-800 mb-1">Preventing Order Clashes</p>
                <p>
                  Do not place multiple orders on the same link simultaneously. Wait until the first order status changes to <strong>COMPLETED</strong> before submitting another order on that target URL.
                </p>
              </div>
            </div>
            ...
            {!providerCanFulfill && (
              <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-700 text-xs flex items-start gap-3">
                ...
                <div>
                  <p className="font-bold">{providerCheckError}</p>
                  <button
                    type="button"
                    onClick={(e) => {
                      e.preventDefault();
                    ...
                    className="underline font-extrabold mt-1 text-rose-800 hover:text-rose-900 cursor-pointer block"
                  >
                    Refresh the page
                  </button>
                </div>
            ...
            <button
              ...
              className="w-full py-3.5 px-6 rounded-2xl bg-primary hover:bg-primary-hover text-white text-sm font-black shadow-md shadow-primary/25 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {orderLoading ? (
                <>
