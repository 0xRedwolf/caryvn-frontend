...
export default function AdminTicketDetailPage() {
  ...
  const loadTicket = async (showLoading = true) => {
    try {
      ...
      const result = await adminApi.getTicket(params.id as string, token);
      if (result.data) {
  ...
  const handleReply = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const result = await adminApi.replyTicket(params.id as string, 'reply', replyMessage, token);
      if (result.data) {
        setReplyMessage('');
        await loadTicket();
      }
  ...
  const handleClose = async () => {
    ...
    try {
      const result = await adminApi.replyTicket(params.id as string, 'close', undefined, token);
      if (result.data) {
        await loadTicket();
      }
    } catch (err) {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize shrink-0 bg-surface-dark border border-border-dark text-text-secondary`}>
              {ticket.priority} Priority
            </span>
          </div>
          <p className="text-text-secondary">
            Opened by {ticket.user_email} on {formatDate(ticket.created_at)}
          </p>
        </div>
          ...
          <div className="flex gap-4 max-w-[85%]">
            ...
            <div>
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-white font-medium">{ticket.user_email}</span>
                <span className="text-text-secondary text-xs">{formatDate(ticket.created_at)}</span>
              </div>
              <div className="bg-surface-darker rounded-2xl p-4 text-text-secondary border border-border-dark whitespace-pre-wrap">
                {ticket.message}
              </div>
            </div>
          </div>
              ...
              <div className={reply.is_admin ? 'items-end flex flex-col' : ''}>
                <div className={`flex items-baseline gap-2 mb-1 ${reply.is_admin ? 'justify-end' : ''}`}>
                  <span className="text-white font-medium">
                    {reply.is_admin ? 'Support Admin' : reply.user_email}
                  </span>
          ...
          <div className="p-4 bg-surface-darker border-t border-border-dark rounded-b-xl">
            <form onSubmit={handleReply} className="flex gap-4">
              ...
              <button
                ...
                className="btn-primary shrink-0 self-end mb-1 px-6 min-w-[120px]"
              >
